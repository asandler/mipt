...
int print_error(int t) {
    switch (t) {
        case ERR_OUT_MEM:
            fprintf(stderr, "parser: Read error: Can't read the input string (probably it's too long)\n");
            return ERR_NOT_OK;
        case ERR_ILLEG_QS:
            fprintf(stderr, "parser: Parse error: Illegal quote sequience\n");
            return ERR_NOT_OK;
        case ERR_CASE_CHARS:
            fprintf(stderr, "parser: Parse error: Input string contains case letters\n");
            return ERR_NOT_OK;
        default:
            return ERR_OK;
    }
...
int char_type(char t) {
    if (strchr(separators.data, t))
        return SEPARATOR;
    if (strchr(puncts.data, t))
        return PUNCT;
    if (strchr(quotes.data, t))
        return QUOTE;
    if (t >= 0 && t < 32)
        return NOT_A_CHAR;
    return LETTER;
