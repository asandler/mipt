...
int LCA(int x, int y) {
    while (a[x].depth != a[y].depth)
        if (a[x].depth > a[y].depth) {
            size_t i = 0;
            while (i < a[x].parents.size() && a[a[x].parents[i]].depth >= a[y].depth)
                i++;
            x = a[x].parents[i - 1];
        } else {
            size_t i = 0;
            while (i < a[y].parents.size() && a[a[y].parents[i]].depth >= a[x].depth)
                i++;
            y = a[y].parents[i - 1];
        }
    while (x != y) {
        size_t i = 0;
        while (i < a[x].parents.size() && i < a[y].parents.size() && a[x].parents[i] != a[y].parents[i])
            i++;
        if (i > 0)
            i--;
        x = a[x].parents[i];
